...
const MenuManager = {
    ...
    renderMenu() {
        ...
        Object.entries(this.filteredData).forEach(([categoryName, products], index) => {
            // Solo mostrar categorías que tengan productos
            if (Object.keys(products).length > 0) {
                const categoryHtml = this.renderCategory(categoryName, products, index);
    ...
    renderProductCard(sku, product) {
        ...
        const description = SanbornsUtils.cleanText(product.description) || 'Sin descripción disponible';
        const productName = SanbornsUtils.cleanText(product.orderPrductName) || 'Producto sin nombre';
    ...
    showProductModal(sku) {
        ...
        $('#productModalImage').attr('src', imageUrl).attr('alt', product.orderPrductName);
        $('#productModalDescription').text(product.description || 'Sin descripción disponible');
    ...
    filterMenu() {
        if (!this.currentSearch) {
            ...
            Object.entries(this.menuData).forEach(([categoryName, products]) => {
                ...
                const categoryMatches = categoryName.toLowerCase().includes(this.currentSearch);
        ...
        if (this.currentSearch) {
            ...
            if (totalResults === 0) {
                SanbornsUtils.showToast(`No se encontraron productos para "${this.currentSearch}"`, 'warning');
            } else {
                SanbornsUtils.log(`Búsqueda: "${this.currentSearch}" - ${totalResults} resultados`);
            }
        }
    },
