...
const SanbornsUtils = {
  ...
  bounceButton($element) {
    $element.addClass('animate-bounceIn').one('animationend', function () {
  ...
  showToast(message, type = 'info', duration = 3000) {
    ...
    $toast.addClass('animate-slideInBottom').fadeIn(300);

    // Auto-hide
    setTimeout(() => {
      $toast.addClass('animate-slideOutBottom').fadeOut(300, function () {
        $(this).remove();
      });
    }, duration);
  },
  ...
  getCurrentSchedules() {
    ...
    for (const [scheduleName, schedule] of Object.entries(config.schedules)) {
      ...
      if (currentHour >= schedule.startHour && currentHour < schedule.endHour) {
        ...
        if (config.debugMode) {
          this.log(
            `Horario activo: ${scheduleName} (${currentHour}:xx)`,
            'info',
            schedule
          );
        }
    ...
    if (config.debugMode) {
      if (activeSchedules.length > 0) {
        this.log(
          `Horarios activos detectados: ${activeSchedules.join(', ')} (${currentHour}:xx)`
        );
      } else {
        this.log(
          `Ningún horario activo para las ${currentHour}:xx - usando menú general`
        );
      }
    }
  ...
  menuNodeExists(menuNode, menuData) {
    ...
    const exists =
      menuData.hasOwnProperty(menuNode) &&
      Object.keys(menuData[menuNode]).length > 0;

    if (this.menuScheduleConfig.debugMode) {
      this.log(
        `Verificando nodo "${menuNode}": ${exists ? '✅ existe' : '❌ no existe'}`
      );
    }
  ...
  navigateToMenuSection(menuNode) {
    ...
    if (
      window.SanbornsApp &&
      typeof window.SanbornsApp.showSection === 'function'
    ) {
    ...
    setTimeout(() => {
      ...
      if ($(categorySelector).length > 0) {
        ...
        if (config.debugMode) {
          this.log(
            `Elemento ${categorySelector} no encontrado - usando fallback`
          );
        }
      }
    }, 500);
