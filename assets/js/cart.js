...
const CartManager = {
  ...
  initViewToggle() {
    ...
    const savedView =
      SanbornsUtils.getFromStorage('sanborns-view-preference') || 'cards';
  ...
  addProduct(product, quantity = 1) {
    ...
    SanbornsUtils.showToast(
      `${product.orderPrductName} agregado al carrito`,
      'success'
    );
  ...
  updateQuantity(sku, newQuantity) {
    ...
    if (item.estado !== 'nuevo') {
      SanbornsUtils.showToast(
        'No se puede modificar producto ya enviado a cocina',
        'warning'
      );
      return false;
    }
  ...
  removeProduct(sku) {
    ...
    if (item.estado !== 'nuevo') {
      SanbornsUtils.showToast(
        'No se puede eliminar producto ya enviado a cocina',
        'warning'
      );
      return false;
    }
   ...
   */
  updateCuentaButton() {
    const hasOrderedItems = this.cart.items.some(
      item => item.estado === 'enviado_cocina' || item.estado === 'servido'
    );

    const $cuentaButtonMobile = $(
      '.mobile-nav .nav-item[data-section="cuenta"]'
    );
    const $cuentaButtonDesktop = $(
      '#desktop-header .nav-link[data-section="cuenta"]'
    );
    ...
    if (hasOrderedItems) {
      ...
      SanbornsUtils.log(
        '✅ Botón "Cuenta" habilitado - hay productos enviados a cocina'
      );
    } else {
      ...
      SanbornsUtils.log(
        '🔒 Botón "Cuenta" deshabilitado - no hay productos enviados a cocina'
      );
    }
  },
  ...
  updateCartSection() {
    ...
    const currentSection = window.SanbornsApp
      ? window.SanbornsApp.currentSection
      : 'mi-orden';
  ...
  updateCartSummary() {
    $('#subtotal').text(
      SanbornsUtils.formatPrice(this.cart.subtotal).replace('$', '')
    );
    $('#impuestos').text(
      SanbornsUtils.formatPrice(this.cart.tax).replace('$', '')
    );
    $('#total').text(
      SanbornsUtils.formatPrice(this.cart.total).replace('$', '')
    );
  },
  ...
  updateModalPrice() {
    ...
    $('#addToCartPrice').text(
      SanbornsUtils.formatPrice(total).replace('$', '')
    );
  },
  ...
  async processOrder() {
    ...
    if (!this.hasNewProducts()) {
      SanbornsUtils.showToast(
        'No hay productos nuevos para ordenar',
        'warning'
      );
      return;
    }
  ...
  showServedState() {
    ...
    if (hasProductsInKitchen) {
      // Mostrar mensaje de orden enviada a cocina con mejor fraseo
      $cartSummary.html(`
      ...
      const hasServedProducts = this.cart.items.some(
  ...
  async processPay() {
    ...
    const payableItems = this.cart.items.filter(
      item => item.estado === 'enviado_cocina' || item.estado === 'servido'
    );
  ...
  simulateStates() {
    if (this.cart.items.length === 0) {
      SanbornsUtils.showToast(
        'Agrega productos primero para simular estados',
        'warning'
      );
      return;
    }
  ...
  debug: {
    simulateStates: function () {
      CartManager.simulateStates();
    },
    resetAllToNew: function () {
      CartManager.resetAllToNew();
    },
    getStatusCounts: function () {
      return CartManager.getStatusCounts();
    },
    markAsSent: function () {
      CartManager.markProductsAsSent();
    },
    markAsServed: function () {
      CartManager.markProductsAsServed();
    },
  },
};
